...
Alternatively, there is a package ``readr`` which provides similar functionality, 
but, among other benefits, 

- is 10x-100x faster[^1]. 
- has more consistent naming conventions. 

. . .

Provides ``read_csv()``, ``read_tsv()``, ``read_delim()``, ``read_fwf()``, ``read_table()``, 
and ``read_log()``.

[^1]: This is claimed on the documentation [here](https://www.rdocumentation.org/packages/readr/versions/2.1.3).
...
Avoid manually chaging global state with the library ``withr``.
